...
def verify_anchors() -> dict[str, dict[str, object]]:
    ...
    for role_key, (profile_path, atlas_path, frame_size) in ROLE_ANCHORS.items():
        ...
        foot_edge = visual_offset[1] + bounds[3] - frame_size[1] / 2
        assert abs(foot_edge) < 0.01, (
            f"{role_key} foot edge is {foot_edge}px from actor ground; "
            f"bounds={bounds}, visual_offset={visual_offset}"
        )
        report[role_key] = {
